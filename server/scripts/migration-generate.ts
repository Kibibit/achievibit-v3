...
const command = `npm run typeorm migration:generate -- -d src/data-source.ts src/migrations/${ migrationName }`;
...
try {
  ...
  console.error(bgRed.white('Error generating migration:'));
  console.error(error);
  process.exit(1);
}
