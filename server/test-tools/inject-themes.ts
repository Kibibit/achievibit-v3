...
export async function injectThemes(
  inputDir: string = './test-results',
  projectName: string = path.basename(path.resolve(inputDir))
) {
...
// Run the script
  try {
    await main(inputDir, projectName);
 ...
 */
function injectScriptToHTML(htmlFile, projectName) {
 ...
 * Main function
 */
async function main(inputDir: string, projectName: string) {
  ...
  htmlFiles.forEach((file) => {
    injectScriptToHTML(file, projectName);
  });
...
async function globalTeardown() {
  verbose('Jest global teardown starting - preparing to emit test-results-updated event');
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      verbose(`Attempt ${attempt}/${MAX_ATTEMPTS} to connect to Socket.IO server`);
      ...
      const connected = await new Promise<boolean>((resolve) => {
        ...
        const timeoutId = setTimeout(() => {
          verbose(`Connection attempt ${attempt} timed out after ${CONNECTION_TIMEOUT}ms`);
          resolve(false);
        }, CONNECTION_TIMEOUT);
        ...
        socket.on('connect', () => {
          ...
          verbose('Successfully connected to Socket.IO server');
          resolve(true);
        });
      ...
      if (connected) {
        verbose('Emitting test-results-updated event');
        
        // Use simpler emit without acknowledgement
        socket.emit('test-results-updated', { timestamp: new Date().toISOString() });
        verbose('Event emitted');
        
        // Give the event some time to be processed
      ...
      if (attempt < MAX_ATTEMPTS) {
        verbose(`Waiting ${RETRY_DELAY}ms before next connection attempt...`);
  ...
  // Let's try a direct HTTP request as fallback
  try {
    verbose('Attempting HTTP fallback to refresh test results');
    
    // Using simple fetch to hit a URL that could trigger the refresh
    const response = await fetch('http://localhost:10102/api/test-results-refresh', {
      method: 'POST',
      headers: {
    ...
    // console.log('HTTP fallback response:', response.status);
    process.exit(0);
  } catch (error) {
    console.error('HTTP fallback failed:', error.message);
    verbose('Failed to connect to Socket.IO server after all attempts. Tests results may not refresh automatically.');
  }
}
